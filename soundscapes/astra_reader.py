...
from standard_labels import ASTRALabels, StandardLabels
...
class ASTRADatasetLabels(dsr.DatasetLabels):
    ...
    def __init__(self, al: ASTRALabels = ASTRALabels(), stl: StandardLabels = StandardLabels()):
        ...
        self.standardize_dict = {
            self.astra_labels.audio_data: self.standard_labels.audio_wf,
            self.astra_labels.first_sample_epoch_s: self.standard_labels.t0_epoch_s,
            self.astra_labels.audio_fs: self.standard_labels.audio_fs,
            self.astra_labels.station_id: self.standard_labels.station_id,
            self.astra_labels.station_lat: self.standard_labels.station_lat,
            self.astra_labels.station_lon: self.standard_labels.station_lon,
            self.astra_labels.launch_id: self.standard_labels.event_id,
            self.astra_labels.launch_pad_lat: self.standard_labels.source_lat,
            self.astra_labels.launch_pad_lon: self.standard_labels.source_lon,
            self.astra_labels.reported_launch_epoch_s: self.standard_labels.source_epoch_s}
...
class ASTRAReader(dsr.DatasetReader, dsr.PlotBase):
    ...
    def print_metadata(self):
        ...
        metadata_df = self.compile_metadata(self.labels.astra_labels.launch_id, self.labels.event_metadata)
    ...
    def plot_event(self, launch_id: str = ""):
        ...
        sorted_df = launch_df.sort_values(by=self.astra_labels.est_prop_dist_km)
        # We'll be plotting the waveforms from the launch relative to the mission's reported launch time.
        rep_launch_epoch_s = self.data[self.labels.astra_labels.reported_launch_epoch_s][sorted_df.index[0]]
        ...
        launch_n_srbs = self.data[self.labels.astra_labels.n_srbs][sorted_df.index[0]]
        launch_rocket_type = self.data[self.labels.astra_labels.rocket_type][sorted_df.index[0]]
        launch_rocket_model = self.data[self.labels.astra_labels.rocket_model_number][sorted_df.index[0]]
        ...
        for station in sorted_df.index:
            ...
            audio_data = self.data[self.labels.astra_labels.audio_data][station]
            ...
            start_time = self.data[self.labels.astra_labels.first_sample_epoch_s][station]
            # The sample rate of all the audio data in ASTRA is 800 Hz, but it is also included for convenience
            fs = self.data[self.labels.astra_labels.audio_fs][station]
            ...
            est_prop_distance_km = self.data[self.labels.astra_labels.est_prop_dist_km][station]
            self.plot_single_event(f"{round(est_prop_distance_km, 1)} km", relative_time, audio_data)
            relative_start_toa_estimate = self.data[self.labels.astra_labels.s_aligned_toa_est][station] - rep_launch_epoch_s
            relative_peak_toa_estimate = self.data[self.labels.astra_labels.p_aligned_toa_est][station] - rep_launch_epoch_s
            ...
            if self.show_frequency_plots:
                station_id = f"{self.data[self.labels.astra_labels.station_id][station]} ({est_prop_distance_km:.1f} km)"
    ...
    def plot_recording(self, launch_id: str = "", station_id: str = ""):
        ...
        sorted_df = launch_df.sort_values(by=self.labels.astra_labels.est_prop_dist_km)
        if station_id:
            sorted_df = sorted_df[sorted_df[self.labels.astra_labels.station_id] == station_id]
        station = sorted_df.index[0]
        # We'll be plotting the waveforms from the launch relative to the mission's reported launch time.
        rep_launch_epoch_s = self.data[self.labels.astra_labels.reported_launch_epoch_s][station]
        date_string = (datetime.fromtimestamp(rep_launch_epoch_s, tz=timezone.utc)).strftime("%d %B %Y")
        xlabel = f"Time (s) since launch"
        # For the title, we'll include some information on the launch included in the ASTRA dataset
        launch_n_srbs = self.data[self.labels.astra_labels.n_srbs][station]
        launch_rocket_type = self.data[self.labels.astra_labels.rocket_type][station]
        launch_rocket_model = self.data[self.labels.astra_labels.rocket_model_number][station]
        title = (f"Normalized ASTRA audio data from launch {launch_id} on {date_string}"
                 f"\nRocket: {launch_rocket_type}, {launch_rocket_model} configuration ({launch_n_srbs} SRBs)")
        sa_toa_color, pa_toa_color = self.cbf_colors[0], self.cbf_colors[1]
        print(f"Plotting data from {launch_rocket_type} launch {launch_id} on {date_string}")
