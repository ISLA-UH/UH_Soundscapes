...
from standard_labels import ASTRALabels as AL, StandardLabels as STL
...
class ASTRAReader(dsr.DatasetReader, dsr.PlotBase):
    ...
    def plot_event(self):
        launch_id = self.data[self.dataset_labels.standard_labels.event_id][self.data.index[0]]
        # We'll be plotting the waveforms from the launch relative to the mission's reported launch time.
        rep_launch_epoch_s = self.data[self.dataset_labels.astra_labels.reported_launch_epoch_s][self.data.index[0]]
